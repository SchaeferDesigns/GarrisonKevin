...
export function checkName(input) {
    ...
    if (!value)
        return 'Bitte tragen Sie Ihren Namen ein.';
    if (value.length < limits.name.min)
        return 'Bitte tragen Sie Ihren vollständigen Namen ein.';
    if (value.length > limits.name.max)
        return 'Der Name ist zu lang.';
    if (/\d/.test(value))
        return 'Ein Name enthält keine Ziffern.';
...
export function checkPhone(input, required) {
    const value = tidy(input);
    if (!value)
        return required ? 'Bitte geben Sie eine Telefonnummer an.' : null;
    if (/[a-zA-Z]/.test(value))
        return 'Eine Telefonnummer besteht nur aus Ziffern, zum Beispiel 0170 1234567.';
    ...
    if (isFakeDigits(digits))
        return 'Diese Telefonnummer sieht nicht echt aus.';
    /* Deutsche Vorwahlen beginnen nach der 0 nie mit einer weiteren 0. */
    if (/^00/.test(compact.replace('+49', '0')))
        return 'Nach der Vorwahl-Null folgt keine weitere Null.';
    return null;
}
/* -------------------------------------------------------------- E-Mail */
export function checkEmail(input, required) {
    const value = tidy(input).toLowerCase();
    if (!value)
        return required ? 'Bitte geben Sie eine E-Mail-Adresse an.' : null;
    if (value.length > 120)
        return 'Diese E-Mail-Adresse ist zu lang.';
    if (!/^[a-z0-9!#$%&'*+/=?^_`{|}~-]+(\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@([a-z0-9]([a-z0-9-]*[a-z0-9])?\.)+[a-z]{2,24}$/.test(value)) {
        return 'Diese E-Mail-Adresse sieht nicht vollständig aus.';
    }
    if (placeholderEmails.includes(value))
        return 'Bitte tragen Sie Ihre richtige E-Mail-Adresse ein.';
    const domain = value.slice(value.indexOf('@') + 1);
    const corrected = domainTypos[domain];
    if (corrected)
        return `Meinten Sie ${value.slice(0, value.indexOf('@') + 1)}${corrected}?`;
    return null;
}
/* ------------------------------------------------------------ Ort / PLZ */
export function checkPlace(input) {
    const value = tidy(input);
    if (!value)
        return 'Bitte geben Sie Ort oder Postleitzahl der Baustelle an.';
    if (value.length > limits.place.max)
        return 'Diese Angabe ist zu lang.';
    const postal = value.match(/\d+/g);
    if (postal) {
        const wrongLength = postal.find((group) => group.length !== 5);
        if (wrongLength)
            return 'Eine deutsche Postleitzahl hat fünf Ziffern.';
        const code = Number(postal[0]);
        if (code < 1001 || code > 99998)
            return 'Diese Postleitzahl gibt es nicht.';
    }
    const letters = value.replace(/\d/g, '').trim();
    if (!postal && letters.length < 3)
        return 'Bitte den Ort ausschreiben oder die Postleitzahl angeben.';
...
/* ------------------------------------------------------------- Mengen */
export function checkAmount(input, key) {
    const value = tidy(input);
    if (!value)
        return null;
    const normalized = value.replace(',', '.');
    if (!/^\d+(\.\d{1,2})?$/.test(normalized))
        return 'Bitte nur eine Zahl eintragen, zum Beispiel 32 oder 32,5.';
    const number = Number(normalized);
    const rule = limits[key];
    if (!(number >= rule.min))
        return `Bitte mindestens ${rule.min} ${rule.unit} angeben oder das Feld leer lassen.`;
    if (number > rule.max) {
...
export function checkMessage(input) {
    ...
    if (value.length > limits.message.max)
        return 'Die Beschreibung ist zu lang – bitte kürzen.';
    if (!/\p{L}/u.test(value))
        return 'Bitte beschreiben Sie kurz mit Worten, worum es geht.';
    return null;
}
